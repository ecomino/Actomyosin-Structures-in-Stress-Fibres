...
const B = 3.0 # Right end point
const Δt = 0.1 # Step size
const T = 1 # Number of time steps

# Filament parameters
const N = 3 # Number of actin filaments, use odd number to ensure they do not visually overlap with focal tesions
...
# Motor parameters
const M = 0 # Maximum number of myosin motors
...
function E(x, xn, O_mat, Oᵩ_mat, y)
    ...
    ξ = 1.0 # Drag on filaments due to moving through the extracellular matrix (think about units!)
...
function main(PLOTSIM)
    ...
    # Output results
    PLOTSIM && gif(anim, "connectivity-3.gif", fps=5)
